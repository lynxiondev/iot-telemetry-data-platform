...
> Faceless project - no real truck data, 100% synthetic but production-like.

### Architecture
Generator (Python) -> GitHub Raw (CSV) -> Databricks Lakehouse

-> Bronze: Raw ingestion + audit columns (ingest_timestamp)
-> Silver: Deduplication (Window) + Quality filters
...
├── databricks/
│ └── pipelines/
│ ├── 01_bronze.py # Ingest from GitHub Raw via pandas bridge (Serverless pattern)
│ ├── 02_silver.py # Window deduplication + quality rules
│ └── 03_gold.py # Daily aggregation ready for BI
...
- `total_events`, `avg_speed_kmh`, `avg_battery_pct`, `max/min_engine_temp_c`
...
2. Run `01_bronze.py` -> `02_silver.py` -> `03_gold.py`
3. Check `SELECT * FROM gold_device_daily_summary`

### Next Steps
- [ ] Move raw source to Unity Catalog Volume `/Volumes/...`
